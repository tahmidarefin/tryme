...
export default function MultiChoice({preview, item, id}) {
    ...
    return (
        <>
            ...
            <div className="answer">
                ...
                (<label className="choice"  key={_key} htmlFor={`${data["id"]} ${_key}`}> {(data["correct_answers"].includes(_key) && preview === true ? 
                <input type="checkbox" id={`${data["id"]} ${_key}`} name={`${data["id"]} ${_key}`} value={_key} defaultChecked /> :
                <input type="checkbox" id={`${data["id"]} ${_key}`} name={`${data["id"]} ${_key}`} value={_key} />)}
                    {_value}</label>))}
            </div>
        </div> }
        </>
